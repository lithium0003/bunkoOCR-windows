...
std::vector<int> line_detect(int im_width, int im_height, const std::vector<float>& loc, const std::vector<float> &lineimage, const std::vector<float>& sepimage)
{
    ...
    for (int i = 0; i < boxcount; i++) {
        ...
        boxes[i].cx = loc[i * 9 + 1];
        boxes[i].cy = loc[i * 9 + 2];
        boxes[i].w = loc[i * 9 + 3];
        boxes[i].h = loc[i * 9 + 4];
        boxes[i].code1 = loc[i * 9 + 5];
        boxes[i].code2 = loc[i * 9 + 6];
        boxes[i].code4 = loc[i * 9 + 7];
        boxes[i].code8 = loc[i * 9 + 8];
        // ルビ親文字
    ...
    for (int i = 0; i < boxes.size(); i++) {
        ...
        result[i * 5 + 0] = boxes[i].id;
        result[i * 5 + 1] = boxes[i].block;
        result[i * 5 + 2] = boxes[i].idx;
        result[i * 5 + 3] = boxes[i].subidx;
        result[i * 5 + 4] = boxes[i].subtype;
    }
    return result;
}
